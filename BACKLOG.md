...
우선순위: **P0** = 긴급/지금 착수 · **P1** = 높음 · **P2** = 중간 · **P3** = 낮음.

## 열린 항목

- [ ] **[P1] API 크레딧 확보 후 자동 triage 재개** — `console.anthropic.com`에서 API 크레딧
...
- [ ] **[P3] `~/.local/bin` PATH 멱등성 체크 Mac/Linux 통일** — Mac은 3개 rc 파일을 grep,
  Linux는 처리 중인 단일 cfg만 grep. 동작엔 문제없으나 시그니처를 블록 주석 마커 등 더
  구체적인 값으로 바꿔 견고화(현재는 bare `.local/bin` 문자열이라 zoxide 블록과 겹칠 여지).

- [ ] **[P3] install.ps1 Windows Terminal `settings.json` 트레일링 콤마 대응** — WinPS 5.1의
